<template lang="html">
  <div id="CorpLetterDetail">
    ...
    <mt-cell id="huiFu" label="回复结果" v-if="info.status != '0'">
      <mt-radio id="yesN" v-model="info.status" :options="[{ label: '接受', value: '1', disabled: true }, { label: '拒绝', value: '2', disabled: true }]">
      </mt-radio>
    </mt-cell>

    <mt-cell id="huiFu" label="回复结果" v-else>
      <mt-radio id="yesN" v-model="form.status" :options="[{ label: '接受', value: '1' }, { label: '拒绝', value: '2' }]"> </mt-radio>
    </mt-cell>
    ...
    <mt-button
      type="primary"
      style="margin-top:30px !important; height:35px !important; line-height:35px !important; margin-bottom:30px !important;"
      v-if="info.status == '0'"
      size="large"
      @click.prevent="toLetterReply()"
      >回复</mt-button
    >
  </div>
</template>
